                    ...
                    using (SqlCommand command = new SqlCommand("SELECT * FROM ReadingTest", connection))
                    {
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                if (text.Equals("1")) 
                                {
                                    literal2.Text = "<div> Difficulty : Beginner </div>";
                                } else if(text.Equals("2"))
                                {
                                    literal2.Text = "<div> Difficulty : Intermediate </div>";
                                } else if(text.Equals("3"))
                                {
                                    literal2.Text = "<div> Difficulty : Advanced </div>";
                                } else
                                {
                                    literal2.Text = "<div> Difficulty : ??? </div>";
                                }
